<template>
    <div class="defect_block lock">
        ...
        <button v-if="data.item.defect_count > 0" @click="production_show_comments(data.item)" class="open_modal_button">
Показать замечания ({{data.item.defect_count}})
</button>
        <div v-if="modal.comment_list.visible" class="modal-wrapper">
            <div class="modal-block new_task_modal lock">
                ...
                <div class="insblock_content">
                    <div class="comment_area" v-for="comment in data.item.comments" :key="comment.id">
                        <div class="comment_row">
                            <span class="bold created">{{comment.создание + ' ' + '(' + comment.Сотрудник + ')' + ':'}}</span>
                            {{comment.примечание}}
                        </div>
                        <my-attachments :data="{empty: comment.Количество_файлов == 0 , items: comment.defect_attachments || []}" @load-attachments = "load_attachments(comment, data.item)" @hide-attachments = "hide_attachments(comment)" @open-attachment = "open_attachment($event, comment)" @add-attachment="add_attachment_to_comment(comment, data.item)" @del-attachment = "delete_attachment_on_comment($event, comment)"></my-attachments>
                    </div>
                </div>
                <button class="button button-cancel button_global" @click="hide_comments">
Назад
</button>

                <my-modal-alert :data="modal.alert"></my-modal-alert>
                
            </div>
        </div>
        <!-- окно для создания пометки для изделия -->
        <button  @click="show_create_comment" class="open_modal_button">
Добавить замечание
</button>
        ...
        <div v-if="modal.create_comment.visible" class="modal-wrapper">
            <div class="modal-block new_task_modal">
                ...
                <div @click="production_defect_add_attachment(data.item)" class="attach_file_modal">
                    <i class="fas fa-paperclip button_attach_item"></i>
                    <button class="button_sheet button_global">
Прикрепить файл
</button>	
                </div>
                <div v-if="data.item.list_files != 0" class="attached_files">
                    <div v-for="(list_file, index) in data.item.list_files" class="file_row" :key="index">
                        <div class="file_title">
{{list_file.name}}
</div>
                        <i @click="production_defect_deattach(data.item, index)" title="Удалить вложение" class="fas fa-trash-alt button_attach_item"></i>
                    </div>								
                </div>							
                <button @click="production_add_defect_comment(data.item)" class="button_interface">
Сохранить пометку
</button>
                
                

                <button class="button button-cancel" @click="hide_create(data.item)">
Отмена
</button>
            </div>
        </div>
...
*/
    import my_modal_alert from "@/components/my_modal_alert.vue"
    import my_attachments from "@/components/my_attachments.vue"

    export default {

        components: {
            'my-modal-alert': my_modal_alert,
