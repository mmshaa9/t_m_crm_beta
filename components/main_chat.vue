<template>
    <div class="chat_app" >
        <div  class="window">
            <button class="button_interface" @click="show_with_attach_value = true">Показать вложения</button>
        </div>
        <div class="list_wrapper" ref="list_wrapper">
            <div id="list" class="list">
                <div  class="day" v-for ="day in message_list">
                    <div class="date_title">{{day.date}}</div>	
                    <div v-for="message in day.messages" :message="message" :cur_user_id="cur_user_id" >
                        <div class="message_box"  :class="{ right: message.user_id == cur_user_id, left: message.user_id != cur_user_id}">
                            ...
                            <div class="message_body">
                                <div class="message_window">
                                    <div v-if="message.user_id != cur_user_id" class="message_title">{{message.user_name}}</div>
                                    <div v-else class="message_title">Я</div>
                                ...
                                <div class="attach"  v-for="file in message.files2">
        ...
        <div class="controls2">				
            ...
            <button class="button_global" @click="postNewMessage">Отправить</button>
        </div>
        <div  v-show="show_with_attach_value" class="aa1">
            <div class="aa2">
                <div  v-for ="day in message_list" :day="day">
                    <div v-for="message in day.messages" :message="message" :cur_user_id="cur_user_id">
                        <div class="aa3"  v-for="file in message.files2">
                ...
                <button @click="show_with_attach_value = false" class="btn_hide_attachments button_global" >Скрыть вложения</button>
            </div>
        </div>
        <b-loading v-if="convs_id != '0' || convs_id != 0" :is-full-page="false" v-model="isLoading" :can-cancel="false"></b-loading>
    </div>
</template>
...
export default ({
    ...
    data() {
        return{
            cur_conversation_id: this.convs_id,
            cur_user_id: this.user,
            message_list:[],	
            new_message:'',
            isLoading: false,
        ...
        */
        ...
        attach: function(m){
            ...
            deattach: function (f) {
                // открепляем вложение
                axios
                .get(this.url, {params: {cmd: 'deattach', file_id: f.ФайлКод, user_id: this.cur_user_id, t: new Date()}})
                .then((response) =>{                                        
                    this.loadData()
                })
            },
