...
<template>
    <div class="attachments-block">
        <button class="btn" style="color:blue;" v-if="!data.empty && data.items.length == 0" @click="$emit('load-attachments')">Показать вложения</button>                
        <button class="btn hide_attachment_par" v-if="data.items.length>0" @click="$emit('hide-attachments')">Скрыть вложения</button>                
        <button :disabled="data.disabled ?? false" :class="{disabled: data.disabled ?? false}" class="btn" @click="$emit('add-attachment')">Прикрепить вложение</button>
        <div class="attachments-items" v-if="data.items.length>0">
            <div v-for="(item , index) of data.items" v-bind:key="index">
                <button id="open-attachment" class="attachment-name" @click="$emit('open-attachment', item)" :link="item.url2">{{(index+1) + '. ' + item.name}}</button>
